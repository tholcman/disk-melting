...
mdadm --create --verbose /dev/md0 --level=0 --raid-devices=8 /dev/nvme0n1 /dev/nvme0n2 /dev/nvme0n3 /dev/nvme0n4 /dev/nvme0n5 /dev/nvme0n6 /dev/nvme0n7 /dev/nvme0n8
mkfs.xfs /dev/md0
...
add-apt-repository ppa:scylladb/ppa
curl -o /etc/apt/sources.list.d/scylla.list -L ${SCYLLA_REPO_URL}

apt update

apt install -y scylla

read -r -d '' SCYLLA_CFG <<'EOFSCYLLACFG'
...
EOFSCYLLACFG
echo "$SCYLLA_CFG" >/etc/scylla/scylla.yaml

echo "dc=GCP" >/etc/scylla/cassandra-rackdc.properties
echo "rack=europe-west1-b" >>/etc/scylla/cassandra-rackdc.properties


systemctl enable scylla-server.service

chown scylla:scylla /var/lib/scylla
chown scylla:scylla /var/lib/scylla/data
chown scylla:scylla /var/lib/scylla/commitlog

scylla_io_setup
scylla_bootparam_setup
scylla_coredump_setup
scylla_sysconfig_setup --nic eth0
scylla_sysconfig_setup --nic eth1
scylla_cpuscaling_setup
scylla_ntp_setup
node_exporter_install

echo "CPUSET=\"--cpuset 1-15,17-31\"">/etc/scylla.d/cpuset.conf

read -r -d '' PERF_CFG <<'EOFPERFCFG'
cpu_mask: '0xffffffff'
mode: sq_split
nic: ${IP}
tune:
- net
EOFPERFCFG
echo "$PERF_CFG" >/etc/scylla.d/perftune.yaml

service scylla-server start

read -r -d '' STRESSTEST <<'STRESSTEST'
export NODE=${IP}
